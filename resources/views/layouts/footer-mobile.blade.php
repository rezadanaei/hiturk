<style>
    ...
    .footer-mobile-main .active-footer-mobile{
        color: #E3B000 !important;
    }
    .footer-mobile-main .active-footer-mobile svg {
        fill: #E3B000 !important;
    }
    #footer-mobile-button-main {
        all: unset;
        display: flex;
    ...
    .categories-popup-main.open-footer-mobile{
        top:0;
        z-index: 1001;
    }
    .categories-popup-header-main{
        top: 0;
        padding:15px 20px ;
        box-sizing: border-box;
        display: flex;
    ...
    .categories-popup-header-main button{
        all: unset;
    }
...
<div class="footer-mobile-main">
    <a href="{{route('home')}}" class="footer-mobile-link-main {{ Route::currentRouteName() == 'home' ? 'active-footer-mobile' : '' }} ">
    ...
    <button id='footer-mobile-button-main' type="button" class="footer-mobile-items-main "  onclick="toggleActiveFooterMobile()">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <a href="{{route('user.orders')}}" class="footer-mobile-link-main {{ Route::currentRouteName() == 'user.orders' ? 'active-footer-mobile' : '' }}">
    ...
    <a href="{{route('user.dashboard')}}" class="footer-mobile-link-main {{ Route::currentRouteName() == 'user.dashboard' ? 'active-footer-mobile' : '' }}">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<div class="categories-popup-main ">
    <div class="categories-popup-header-main" >
        ...
        <button type="button"  onclick="toggleActiveFooterMobile()">
    ...
    <div class="data-list-main">
        @include('layouts.category-dropdown', ['device' => 'mobile'])
    </div>
</div>
<script>
  // Toggle active state for footer-mobile button and manage popup visibility
  function toggleActiveFooterMobile() {
    const button = document.getElementById('footer-mobile-button-main');
    const currentActive = document.querySelector('.footer-mobile-main .active-footer-mobile');
    const categoriesPopup = document.querySelector('.categories-popup-main');
    ...
    if (button.classList.contains('active-footer-mobile')) {
      ...
      if (toggleActiveFooterMobile.lastActiveElement) {
        toggleActiveFooterMobile.lastActiveElement.classList.add('active-footer-mobile');
        toggleActiveFooterMobile.lastActiveElement = null;
      }
      ...
      if (currentActive && currentActive !== button) {
        toggleActiveFooterMobile.lastActiveElement = currentActive;
        currentActive.classList.remove('active-footer-mobile');
      }

      // Activate the button
      button.classList.add('active-footer-mobile');
    ...
    updateDataListHeightFooterMobile();
  }

  // Select necessary DOM elements
  const footerMobile = document.querySelector('.footer-mobile-main');
  const popupHeader = document.querySelector('.categories-popup-header-main');
  const dataList = document.querySelector('.categories-popup-main .data-list-main');

  // Update height of data list based on screen and visible elements
  const updateDataListHeightFooterMobile = () => {
  ...
  window.addEventListener('resize', updateDataListHeightFooterMobile);
</script>